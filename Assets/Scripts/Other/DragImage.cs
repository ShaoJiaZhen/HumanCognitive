...
public class DragImage : MonoBehaviour {
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            if (CheckGameObject())
            {
                offset = dragGameObject.transform.position - cameraObj.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, targetScreenPoint.z));
            }
        }
        ...
        if (isClickCube)
        {
            ...
            //把当前鼠标的屏幕坐标转换成世界坐标
            Vector3 curWorldPoint = cameraObj.ScreenToWorldPoint(curScreenPoint);
    ...
    bool CheckGameObject()
    {
        ...
        if (Physics.Raycast(ray, out hitInfo, 100f, canDrag))
        {
            ...
            Debug.Log("测试");
            targetScreenPoint = cameraObj.WorldToScreenPoint(dragGameObject.position);
            return true;
        }
        return false;
    }
